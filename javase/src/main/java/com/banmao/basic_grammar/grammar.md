...
字符编码说明：
ASCII码：只有英文、数字、符号等。占8位，但只用到7位，首位补0，比如010000100
一些常见的ASCII码点对应，比如小写字母a=97,b=98等
GBK字符集：汉字占两个字节，英文、数字占一个字节。
只能包含2万多个汉字，所有不够用
UTF-8字符集：汉字占3个字节，英文、数字占一个字节
推荐使用此字符集。
当编码字符集与解码字符集不同时，会出现中文乱码。
但是字母、数字可能正常，因为常见的字符集都支持ASCII编码
...
108、集合
如果再次遍历集合，需要重置迭代器
如果当前容量为0，则初始扩容为10个大小。
ArrayList的扩容是按照1.5倍扩容，如果1.5倍不够，则直接使用当前需要的容量进行扩容。
比如当前大小为8，需要为14个元素，但是扩容后才12个空间。则当前扩容为14个元素。
...
new Runtime().availableProcessors();//可以查看有几个CPU

疑问：1、为什么是调用start而不是run方法呢？

2、继承Thread与实现Runnable的区别？
从java设计来说，继承Thread与实现Runnable，从JDK实现来说，没有啥区别，都是通过调用底层的start0来实现。
建议用实现Runnable方式，因为此种方式可以避免单继承限制。
...
线程常用方法：
1、start底层会创建新的线程，调用run，run就是一个简单的方法调用，不会启用新线程。
2、线程优先级的范围：
/**
...
线程常用方法2：
yield：线程的执行cpu的让步，但不一定能让步成功，得看操作系统的工作状态
...
3、常见的守护线程：垃圾回收机制
守护线程的应用场景：当需要监控其它线程，或作为一个其它线程信息的收集线程，则可以将线程设置为守护线程

线程的生命周期：
官方文档6种状态：
NEW、RUNNABLE（细化为READY、RUNNING）、BLOCKED（阻塞）、WAITING、TIME_WAITING、TEMINATED
![](.grammar_images/线程的状态.png)

Synchronized关键字：
线程同步机制：
保证在任何同一时刻，最多有一个线程访问。以保证数据的完整性
也可以这样理解：线程同步，即当有一个线程在对内存进行操作时，其它线程都不可以对这个内存地址进行操作，直到该线程完成操作。
同步的实现方法：简单实现之Synchronized
1、同步代码块：
synchronized(对象) { // 得到对象的锁，才能操作同步代码
    //需要被同步代码
}
2、同步方法
synchronzied还可以放在方法声明中，表示整个方法为-同步方法。
public synchronized void m(String name) {
    //需要被同步代码
}

同步的原理：
锁是在对象上，在对象的内存中，分为3块数据（对象头、类指针、实例数据）
锁的标记是在对象头中。
锁加在同步方法上，这时锁加的是this对象上。
锁加在静态方法上，这时锁加的是类上。
重要：锁不仅可以加在this上，还可以加在其它对象上。
...
当前情况下，会释放
3、当前线程在同步代码块、同步方法中遇到了未处理的Error或Exception。导致异常结束
4、当前线程在同步代码块、同步方法中执行了线程对象的wait方法。当前线程暂停，并释放锁

不释放锁的操作：
1、线程执行同步代码块、同步方法时，程序调用Thread.sleep()、Thread.field()方法时，不会释放锁
2、线程执行同步代码块时，其它线程调用了该线程的suspend()方法将该线程挂起，该线程不会释放锁
// 提示：应尽量避免使用suspend()和resume()来控制线程。方法已过时
...
处理流中使用了装饰器模式（Decorator），通过包装其它的节点流进行增加操作

重要：java通过字符流读取文件，默认是按utf-8来读取的，
可以知道utf-8的编码方式只有四种：
0xxxxxxx
110xxxxx 10xxxxxx
1110xxxx 10xxxxxx 10xxxxxx
11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
所以读取二进制文件的时候，发现1xxxxxxx，就蒙蔽了，因为字符集编码表中不存在这种编码方式，于是就会解析出错。
所以这里也需要理清一个概念：读取字符时是根据编码方式来的，java中的字符读取并不是两个字节两个字节来读取的。
（utf-8中）比如当遇到0xxxxxxx，就取一个字节就ok，遇到1110xxxx开头，就要取三个字节了。

序列化与反序列化：
序列化就是在保存数据时，保存数据的值和数据类型
反序列化就是在恢复数据时，恢复数据的值和类型
要让某个对象支持序列化机制：让类支持可序列化的方式：实现以下两个接口之一：
Serializable
Externalizable

对象处理流的使用细节：
1、读写顺序需要一致
2、要求实现序列化或反序列化对象，要实现Serializable接口
3、序列化中的类建议添加SerialVersionUID（序列化版本号），增加版本兼容性
4、默认会将所有属性序列化，除了static与transient修饰的属性
5、序列化对象时，要求里面的属性也要实现序列化的接口
6、序列化支持继承性
